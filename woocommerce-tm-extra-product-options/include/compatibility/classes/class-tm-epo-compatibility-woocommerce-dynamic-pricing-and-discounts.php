...
final class TM_EPO_COMPATIBILITY_woocommerce_dynamic_pricing_and_discounts {
	...
	public function get_RP_WCDPD( $field_price = NULL, $product, $cart_item_key = NULL ) {
		...
		if ( class_exists( 'RP_WCDPD' ) && class_exists( 'RP_WCDPD_Pricing' ) ) {
			...
			}/*elseif ($dpd_version_compare2) {
				$product_pricing = RP_WCDPD_Settings::get('product_pricing');
				if (is_array($product_pricing)){
					foreach ( $product_pricing as $rule_key => $rule ) {
						if ( $rule['method'] == 'bulk' && $validated_rule = RP_WCDPD_Promotion_Volume_Pricing_Table::get_instance()->get_applicable_volume_rule($product) ) {
							$selected_rule = $validated_rule;
						}
					}
				}
			...
			if ($dpd_version_compare2){
				$price = array();
				if (!class_exists('RP_WCDPD_Promotion_Volume_Pricing_Table') && file_exists(RP_WCDPD_PLUGIN_PATH . 'includes/classes/promotion/rp-wcdpd-promotion-volume-pricing-table.class.php')){
					include RP_WCDPD_PLUGIN_PATH . 'includes/classes/promotion/rp-wcdpd-promotion-volume-pricing-table.class.php';
				}
				$all_rules = array();
				if (!$product->is_type('variable') && !$product->is_type('variation')) {
					$rule = RP_WCDPD_Promotion_Volume_Pricing_Table::get_instance()->get_applicable_volume_rule($product);
		            ...
		            foreach ($product->get_available_variations() as $variation_data) {
		                ...
		                if ($rule = RP_WCDPD_Promotion_Volume_Pricing_Table::get_instance()->get_applicable_volume_rule($variation)) {
		                    $variation_rules[] = array(
		                        'product'   => $variation,
		                        'rule'      => $rule,
		                    );
				...
				foreach ($all_rules as $single) {
					...
					if (RP_WCDPD_Settings::get('promo_volume_pricing_table_missing_ranges') === 'display') {
			            $quantity_ranges = RP_WCDPD_Promotion_Volume_Pricing_Table::get_instance()->add_missing_ranges($quantity_ranges, $_product);
			        }
