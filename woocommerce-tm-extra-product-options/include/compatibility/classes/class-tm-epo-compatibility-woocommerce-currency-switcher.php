...
final class TM_EPO_COMPATIBILITY_woocommerce_currency_switcher {
	...
	public function woocs_fixed_raw_woocommerce_price($fix_price=0, $product=false, $price=null){
		if ($price === null){
			if (!defined('TC_CS_ERROR')){
				define('TC_CS_ERROR',1);
				wc_add_notice( "You are using an unsupported verion of Currency switcher. Prices will not be correct!", 'error' );
			}
			return $fix_price;
		}
		global $WOOCS;
